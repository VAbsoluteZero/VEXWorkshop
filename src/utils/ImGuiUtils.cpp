...
void vex::ImViewHub::onFirstPaintCall(v2f window_size, i32 display_h)
{
    check(ImGui::GetCurrentContext(), "ImGui must be initialized at this point");
    {
        float scaling_f = static_cast<float>(display_h) / ImGuiSizes::g_default_cvs_sz.y;
        scaling_f *= 2;
        scaling_f = (i32)std::ceil(scaling_f);
        ImGuiSizes::g_scale_factor = scaling_f / 2.0f;

        ImGuiSizes::g_cvs_sz = window_size;
    }

    auto scale_font = [sf = ImGuiSizes::g_scale_factor](int def)
    {
        return (int)(def * sf);
    };
    ...
    if (ImGuiSizes::g_scale_factor > 1.25f)
    {
        for (auto& [k, v] : visuals.styles)
        {
            // magic numbers. looks sharper and more compact this way
            v.ScaleAllSizes(ImGuiSizes::g_scale_factor * 0.8f);
        }
    }
}
