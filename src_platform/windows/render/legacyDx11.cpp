...
i32 vp::SdlDx11Application::init(vp::Application& owner)
{
	...
	const u32 g_vbuff_size = 32000 * sizeof(Vertex);
	const u32 g_ibuff_size = 32000 * 4 * 32; // assume 4 indecies use same vertex avg
	tempgeom = new TempPrimitiveGeometryBatch(this->impl->d3d_device, g_vbuff_size, g_ibuff_size);
...
void vp::SdlDx11Application::frame(vp::Application& owner)
{
	...
	// prepare frame
	{
		...
		TmpMeshBuilder plane_mesh = buildPlane({&tempgeom->frame_alloc}, 8, {0, 0}, {1, 1});
		...
		TempPrimitiveGeometryBatch::DrawArgs args{
			.uniform = {.view_proj = view_proj, .col = {1.f, 0.5f, 0.5f, 1.f}},
			.vertshad = impl->vshad_plain,
			.pixshad = impl->pshad_plain,
			.input_layout = impl->sil_plain};

		tempgeom->drawDynMesh(impl, plane_mesh, args);
		//
		demoRoom();
	}
	{
