...
const select_bias = v2f(0.0001, 0.0001); 
...
fn cs_main(@builtin(global_invocation_id) gid: vec3u, @builtin(local_invocation_id) lid: vec3u) {
    ...
    let stride: u32 = (size_1d / 64) + 1;
    let start_idx: u32 = loc_idx * stride;
    if start_idx >= size_1d {
        return;
    }
    let len: u32 = select(size_1d % 64, stride, loc_idx != 63);
    ...
    for (var i: u32 = 0; i < len; i++) {
        ...
        let cell_val: i32 = i32(distances.cells[cur_i] & dist_mask);
        //if cell_val == 0 { continue;}
        let wall_val: i32 = cell_val + 2;
        ...
        for (var j: i32 = 0; j < 8; j++) {
            ...
            let neighbor_wall: bool = (neighbor_val & (~dist_mask)) > 0;
            c_grid[j] = select(i32(neighbor_val & dist_mask), wall_val, neighbor_wall);
        }

        var r: v2f = v2f();
        for (var j: i32 = 0; j < 8; j++) {
            var v: v2f = vecs[j] * f32(sign(cell_val - c_grid[j]));
            //v *= f32(c_grid[j] != 0);
            r += v;
        }
